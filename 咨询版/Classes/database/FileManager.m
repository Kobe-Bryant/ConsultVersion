...
+(void)fileLog:(NSString*)logString{
	NSData *dataToWrite = [[NSString stringWithString:logString] dataUsingEncoding:NSUTF8StringEncoding];
	
	NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
	NSString *documentsDirectory = [[NSString alloc] initWithString:[paths objectAtIndex:0]];
	NSString *path = [documentsDirectory stringByAppendingPathComponent:@"fileName.txt"];
