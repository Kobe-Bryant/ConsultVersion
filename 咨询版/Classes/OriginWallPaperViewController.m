...
- (void)viewDidLoad {
	...
	self.view.backgroundColor = [UIColor colorWithRed:0.8588 green:0.8588 blue:0.8588 alpha:1.0];
...
- (void) showCustomToolbar{
	...
	UIImage *image = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"共用下bar" ofType:@"png"]];
...
{
	[spinnerView startSpinner:scrollview];
	NSLog(@"imgurl %@",index);
		...
		if(iconDownloader.cardIcon.size.width>2.0){ 
			...
			if([FileManager savePhoto:photoname withImage:iconDownloader.cardIcon])
			{
				...
				if (showWhichOriginPic == SHOW_PRODUCT_PIC){
				    ...
				    self.picArray = [DBOperate queryData:T_PIC theColumn:@"pid" theColumnValue:productId withAll:NO];
				}
				else {
...
- (void)getChoosedIndex:(int)actionID chooseIndex:(int)index{
	...
	else if(actionID == SHARE_ACTIONSHEET_ID){
		NSString *linktitle;
		NSString *linkurl;
		if (showWhichOriginPic == SHOW_PRODUCT_PIC) {
